...
class AliyunProvider(BaseProvider):
    ...
    def _refresh_access_token(self) -> bool:
        ...
        try:
            ...
            if resp.status_code != 200:
                print(f'[ERROR] Token 刷新失败: HTTP {resp.status_code}')
                return False

            data = resp.json()

            if 'access_token' not in data:
                print(f'[ERROR] Token 刷新响应无 access_token: {data}')
                return False
            ...
            print(f'[DEBUG] Token 刷新成功，expires_in={expires_in}s')
            return True
    ...
    def check_qr_status(self, ck: str, t: str) -> dict:
        ...
        try:
            ...
            data = resp.json()
            print(f'[DEBUG] QR query 原始响应: {json.dumps(data, ensure_ascii=False)[:500]}')
            content = data.get('content', {}).get('data', {})
            ...
            if qr_code_status == 'NEW':
                ...
                print(f'[DEBUG] CONFIRMED content keys: {list(content.keys())}')
                print(f'[DEBUG] bizExt 长度: {len(biz_ext) if biz_ext else 0}')
                if biz_ext:
                    try:
                        decoded = base64.b64decode(biz_ext).decode('utf-8')
                        print(f'[DEBUG] bizExt 解码: {decoded[:300]}')
                        biz_data = json.loads(decoded)
                        print(f'[DEBUG] bizExt JSON keys: {list(biz_data.keys())}')
                        pds_login = biz_data.get('pds_login_result', {})
                        if not pds_login:
                            print(f'[DEBUG] pds_login_result 不存在, 尝试其他 key')
                            # 尝试其他可能的 key
                            for key in biz_data:
                                if 'login' in key.lower() or 'token' in key.lower():
                                    print(f'[DEBUG] 找到候选 key: {key}')
                        ...
                        if refresh_token:
                            ...
                            print(f'[DEBUG] pds_login keys: {list(pds_login.keys()) if pds_login else "empty"}')
                    except Exception as e:
                        print(f'[ERROR] 解析 bizExt 失败: {str(e)}')
                        import traceback
                        traceback.print_exc()
    ...
    def list_files(self, path: str = 'root') -> List[FileInfo]:
        ...
        try:
            while True:
                ...
                if resp.status_code != 200:
                    print(f'[ERROR] 列出文件失败: HTTP {resp.status_code}')
                    break
        ...
        for file_id in paths:
            try:
                ...
                if resp.status_code in (200, 202, 204):
                    ...
                    print(f'[ERROR] 删除 {file_id} 失败: {error_msg}')
                ...
                last_error = str(e)
                failed.append(file_id)
                print(f'[ERROR] 删除 {file_id} 异常: {str(e)}')
